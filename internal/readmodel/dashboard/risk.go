...
func LoadRiskState(ctx context.Context, st store.RiskPlanQueryStore, pos store.PositionRecord, timelineLimit int) RiskState {
	stopLoss, takeProfits, ok := DecodeRiskLevels(pos.RiskJSON)
...
func LoadRiskPlanTimeline(ctx context.Context, st store.RiskPlanQueryStore, pos store.PositionRecord, limit int) []RiskPlanTimelineItem {
	...
	for _, row := range rows {
		stopLoss, takeProfits, ok := DecodeRiskLevels(row.PayloadJSON)
		if !ok {
			continue
		}
		decoded = append(decoded, decodedRiskPlanHistory{row: row, stopLoss: stopLoss, takeProfits: takeProfits})
	}
	items := make([]RiskPlanTimelineItem, 0, len(decoded))
	for idx, item := range decoded {
		...
		items = append(items, RiskPlanTimelineItem{
			Source:              strings.TrimSpace(item.row.Source),
			Label:               RiskPlanLabel(strings.TrimSpace(item.row.Source), item.row.Version),
			CreatedAt:           createdAt,
...
func RiskPlanLabel(source string, version int) string {
...
func DecodeRiskLevels(riskJSON []byte) (float64, []float64, bool) {
	if len(riskJSON) == 0 {
		return 0, nil, false
	}
...
func IsTightenSource(source string) bool {
	switch strings.ToLower(strings.TrimSpace(source)) {
